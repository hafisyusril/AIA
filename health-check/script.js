...
(function() {
  const frame = document.querySelector('.frame-health');
  if (!frame) return;
  ...
  pageElement.addEventListener('pageActivated', function() {
    ...
    document.addEventListener('mousemove', function(e) {
      ...
      const topContent = document.querySelector('.top-content-health');
      const bottomContent = document.querySelector('.bottom-content-health');
      
      if (topContent) topContent.style.transform = `translateX(${mouseX * 8}px)`;
      if (bottomContent) bottomContent.style.transform = `translateX(${mouseX * -5}px)`;
    });
